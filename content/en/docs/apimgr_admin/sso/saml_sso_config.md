...
- `keycloak` folder:
    ...
    - Sample `service-provider-apiportal.xml` for configuring API Portal SSO. For more details, see
        ...
        in the [API Portal Administrator Guide](/bundle/APIPortal_77_AdministratorGuide_allOS_en_HTML5).
...
4. For example, if you generated a keystore called `sso.jks` with a passphrase `abc123` and an alias called `ssokey`, the settings in `service-provider.xml` would be as follows:
...
5. In the `SamlIdentityProvider` section, update the `metadataUrl` field as detailed in [Specify the IdP by file](#Specify) or [Specify the IdP by URL](#Specify2).
6. In the `SAMLIdentityProvider` section, update the `Mappings` section with the appropriate mapping of IdP attributes to API Manager attributes. For more information on the mapping syntax, see [Mapping syntax](sso_mapping.htm).
7. Update any other fields as required. For more information on the elements in the `service-provider.xml` configuration file, see [Configuration file elements](sso_config_file_ref.htm).
...
[Configure API Portal for single sign-on](/csh?context=2002&product=prod-api-portal-77)
in the [API Portal Administrator Guide](/bundle/APIPortal_77_AdministratorGuide_allOS_en_HTML5).{{< /alert >}}
