...
* Download payload size, when value is entered (in kb) and the response is bigger then the configured value, response is downloaded as file. The defaults value is blank - disable the downloads. Valid only for AMPLIFY rendering tool.
