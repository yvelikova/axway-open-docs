...
2. Update the `CASSANDRA_HOME/conf/cassandra.yaml` file manually or using the `setup-cassandra` script.
    ...
    The `rpc_address` is used for Cassandra-Clients like the API-Manager to connect to the node. This address might be a different one than the `listen_address` in complex deployments.
