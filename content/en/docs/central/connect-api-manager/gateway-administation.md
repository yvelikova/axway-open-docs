...
You can also change the properties values (`apicId`, `subscriptions`) to suit your custom fields pattern. If you do so, be sure you update the agent configuration files accordingly (`proxyApicIDField` and `subscriptionApplicationField`). `apicId` should be editable as it could help to re-discover an API in case of issue by simply remoing its value.
...
* In case of success, the subscriber consumes the API otherwise API cannot be consuned.

In case the application is not available because it is not in the same organisation than the API, you can grant specific access to the API:
