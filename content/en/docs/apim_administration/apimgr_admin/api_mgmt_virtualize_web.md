...
Before you can configure an API in API-Manager to use Two-Way-SSL you must first configure the API-Manager Traffic Port (Default 8065) to accept & trust client certificates issued by the server CA instead of ignoring them by default.  
You can read more how to configure Mutual Authentication and accepting client certificates [here](/docs/apim_policydev/apigw_gw_instances/general_services/#mutual-authentication-settings). 

{{< alert title="Note" color="primary" >}} It is important **not to require** client-certificates, as this would mean all clients for all APIs must provide a client-certificate. Clients not providing a client-certificate will fail during the SSL-Handshake and unable to access your APIs.{{< /alert >}}

Only with that, clients have the option during the SSL-Handshake to send their client-certificate. This is then used by the API-Manager to validate access.  
By default, the provided client certificate must contain a Subject Common Name (CN) containing the API-Key generated by API Manager or a configured external credential for a registered application. The CN value is evaluated at runtime using a selector (e.g. ${certificate.subject.CN}), and used to look up the Client-Registry to retrieve the belonging application. You can configure the following settings:
